---
title: "NYC Summer 2019 Bike Share Use"
author: "Steve Buyske"
...
I want to start by looking at 2019 only. We will include a `day` variable, which is day of the year, and also separate intercepts for the day of the week, using a hierarchical model. The argument `control = list(adapt_delta = 0.999)` is something to fix a problem that we won't discuss for a few weeks.

```{r 2019 only}
citi_hier_2019 <- stan_glmer(rides ~ day + (1 | day_of_the_week), data = citibike2019, control = list(adapt_delta = 0.999))
```
...
```{r 2019 only 2nd try}
citi_hier_2019 <- stan_glmer(rides ~ day + (1 | day_of_the_week), data = citibike2019, iter = 5000, control = list(adapt_delta = 0.999), cores = 4)
```
...
```{r}
launch_shinystan(citi_hier_2019)
```

```{r}
pp_check(citi_hier_2019)
```

The pp_check is disappointing. The simulations are more likely to have both high numbers of rides as well as average numbers of rides. I suspect we need to incorporate weather into the model, but for learning purposes we will ignore these problems.
...
```{r}
plot(citi_hier_2019, plotfun = "areas_ridges", regex_pars = "Intercept.+day.of.the.week") 
```
...
```{r}
model_parameters(citi_hier_2019, ci = 0.9, rope_ci = 0.9, rope_range = c(-50, 50))
```
We can see the group effect by adding the argument `effects = "random"` to the function call.


```{r}
model_parameters(citi_hier_2019, ci = 0.9, rope_ci = 0.9, rope_range = c(-50, 50), effects = "random")
```

You can see that from the `pd` column that we are most confident that Saturday and Sunday are to one side of zero. Notice also that the effect is negative---fewer riders---while in the Bay Area we saw last there were more on the weekend. You can see that only for Sunday would the ROP + HDI decision lead to "reject," while all the others would be "undecided."
...
citi_hier_2019 %>%
  ...
  names()
```
What is the probability that summer Sundays in 2019 had fewer rides than Mondays? We need to be able to write out the condition, and because the names are not legal R variable names we have to use back ticks.
...
It may take a while to get the syntax right, but once you are used to it the calculations are surprisingly quick to do.