...
#You can use set.seed(12345) or some other permutation to create 
...
# rbinom(n, size, prob)  binomial distribution where n is the number of 
# random numbers, size is the number of trials and prob is the probability of 
# a heads (pi) 
...
if (FALSE)
{"
You can also use the uniform random variable generator to generate 
probability distributions. x=ranuni(0);
y=F-1(x) y ~ F  so if F-1 is the inverse exponential then y is ~ exponential
Random number generators are used in simulations.
Pharmacoeconomic models use discrete event simulation.  
So patients are followed as events 
