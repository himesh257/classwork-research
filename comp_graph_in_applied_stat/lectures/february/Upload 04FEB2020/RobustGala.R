...
bcoef <- matrix(0,1000,6)  #generate a 1000 by 6 matrix of 0s which will be populated with the regression results
head(bcoef)

for(i in 1:1000){
newy <- predict(LADmodel) + residuals(LADmodel)[sample(30,rep=T)]  #take a sample of residuals and generate new y values
